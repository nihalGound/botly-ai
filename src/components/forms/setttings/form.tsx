...
const SettingForm = ({ id, name, plan, chatBot }: Props) => {
      ...
      <div className="flex flex-col gap-3 mt-5">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="flex gap-1 bg-cream rounded-full px-3 py-1 text-xs items-center font-bold">
            <PremiumBadge />
            <span className="dark:text-black">Premium</span>
          </div>
        </div>
        <Separator orientation="horizontal" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="col-span-1 flex flex-col gap-5 order-2 md:order-1">
            <EditChatbotIcon
              chatBot={chatBot}
              register={register}
              errors={errors}
