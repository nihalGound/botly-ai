...
export const AddDomainSchema = z.object({
    domain: z
        .string()
        .min(4, { message: "A domain must have atlease 3 characters" })
        .refine(
            (value) =>
...
export const DomainSettingsSchema = z.object({
    ...
    welcomeMessage: z
    .string()
    .min(6, "Them message must be atleast 6 characters")
    .optional()
    .or(z.literal('').transform(() => undefined)),
})
.refine(
    (schema) => {
        if (schema.image?.length) {
            if (
                ACCEPTED_FILE_TYPES.includes(schema.image?.[0].type) && schema.image?.[0].size <= MAX_UPLOAD_SIZE
            ) {
                return true
            }
        }
    ...
    {
        message: 
            "The file must be less than 2MB, and on PNG, JPEG & JPG files are accepted",
            path: ['image']
    }
)
